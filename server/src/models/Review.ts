...
const reviewSchema = new Schema({ author: { type: Schema.Types.ObjectId, ref: 'User', required: true }, consultancy: { type: Schema.Types.ObjectId, ref: 'Consultancy', required: true }, rating: { type: Number, min: 1, max: 5, required: true }, body: { type: String, required: true, trim: true, maxlength: 1500 }, status: { type: String, enum: ['pending', 'published', 'flagged', 'hidden'], default: 'published' } }, { timestamps: true });
